...
/**
 * Morpheus user interface. Builds the base ui, keeps
 * track of registered components and provides an API
 * to inject new UI parts.
 */
morpheus.ui = (function($) {
	...
	me.init = function() {
		...
		// This is hooked here because if we hook before DOM has loaded,
		// IE crashes.
	...
	me.addPage = function(key, page) {
		...
		me.pages[key] = {obj:page,element:page.getPage()};
	...
	me.showPage = function(key) {
		if( typeof(me.pages[key]) === "undefined" ) {
			...
			if( key !== me.DEFAULT_PAGE ) {
				$.bbq.pushState({ p: me.DEFAULT_PAGE });
			}
		} else {
...
morpheus.ui.mainmenu = (function($) {
	 ...
	 */
	me.render = function() {
		me.container.processTemplate({items:me.menuItems});
	};
	...
	me.addItem = function(name, page, data) {
		var data = data || {};
		me.menuItems.push({ name:name, page:page, data:data });
