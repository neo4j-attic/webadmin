morpheus.ui.MainMenu = ( function( $, undefined )
{
    ...
    me.render = function()
    {
        ...
        if( me.container !== null ) {
            ...
            var currentServices = [];
            if( morpheus.Servers.getCurrentServer() ) {
                if(!morpheus.Servers.getCurrentServer().manage.servicesLoaded()) {
                    morpheus.Servers.getCurrentServer().bind("services.loaded",me.render);
                } else {
                    currentServices = morpheus.Servers.getCurrentServer().manage.availableServices();
                }
            }
    ...
    neo4j.events.bind("morpheus.servers.current.changed", function(ev) {
        if(ev.data.current) {
            morpheus.ui.MainMenu.setPerspective("server");
        }
    });
